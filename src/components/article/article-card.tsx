...
export function ArticleCard({ post, orientation = "vertical" }: ArticleCardProps) {
  ...
  return (
    <Link href={`/posts/${post.slug}`} className="block">
      <Container
        className="group flex w-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg sm:rounded-[2.25rem] cursor-pointer"
      >
        {post.coverImage ? (
